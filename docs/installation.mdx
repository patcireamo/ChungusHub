...
The port, the interface and both folder locations are read from `chungushub.config.json`, which
ChungusHub writes beside the executable on its first run (beside the repository when you run from
source):
...
Set one and it wins over the file, which wins over the defaults above. Useful for a launcher
script or a one-off run; anything permanent belongs in the file.
