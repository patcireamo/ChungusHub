...
`openBrowser` is what a downloaded build does as it starts. Set it to `false` and it serves
without throwing a tab up, which is what a machine you are not sitting at wants. A copy run from
source opens nothing either way.
